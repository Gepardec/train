...
public class EC2Stack extends Stack {
    ...
    public EC2Stack(final Construct scope, final String id, final StackProps props) {
        ...
        for (int i = 0; i < config.instanceCount; i++) {
            ...
            CfnOutput.Builder.create(this, config.indexed("instanceIP", i)).value(instance.getInstancePublicIp()).build();
            CfnOutput.Builder.create(this, config.indexed("instanceURI", i)).value(instance.getInstancePublicDnsName()).build();
        }
    }
